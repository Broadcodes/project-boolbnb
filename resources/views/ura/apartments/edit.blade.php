        ...
        <form action="{{ route('ura.apartments.update', $apartment->apartment_slug) }}" method="POST"
            ...
            <div class="p-3">
                ...
                <input type="text" name="apartment_title" id="apartment_title"
                    value="{{ old('apartment_title', $apartment->apartment_title) }}" required maxlength="255">
            ...
            <div class="p-3">
                ...
                <textarea name="description" id="description" cols="30" rows="10" required minlength="5"> {{ old('description', $apartment->description) }} </textarea>
                @error('description')
                    <h5 class="text-danger ml-4">{{ $message }}</h5>
                @enderror
            </div>
            ...
            <div class="p-3">
                ...
                <select name="bed" id="bed">
                    @for ($i = 1; $i <= 10; $i++)
                        <option value="{{ $i }}" {{ $apartment->bed == $i ? 'selected' : '' }}>
                            {{ $i }}
                        </option>
                    @endfor
                </select>
            ...
            <div class="p-3">
                ...
                <input type="number" name="sqm" id="sqm" step=".01"
                    value="{{ old('sqm', $apartment->sqm) }}" required max="999.99">
                @error('sqm')
                    <h5 class="text-danger ml-4">{{ $message }}</h5>
                @enderror
            </div>
            ...
            <div class="p-3">
                ...
                <input type="number" name="price" id="price" step=".01"
                    value="{{ old('price', $apartment->price) }}" required max="999999.99">
                @error('price')
                    <h5 class="text-danger ml-4">{{ $message }}</h5>
                @enderror
            </div>
...
<h5 class="text-danger
        ...
        <div class="p-3">
            ...
            <input type="number" name="civic_number" id="civic_number"
                value="{{ old('civic_number', $apartment->civic_number) }}" required>
            @error('civic_number')
                <h5 class="text-danger ml-4">{{ $message }}</h5>
            @enderror
        </div>
